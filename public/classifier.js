// Gauge gauge-0.3.0; https://github.com/marlandoj/gauge@49b2aa5cc4abd11c98ce35b0b6fd9d2ee9c66621
// Verbatim TypeScript source SHA-256: 6d6bbc69cdb5eaca016516eeaf9de6062aa3a4043d24401034b935ac3f29776e
// Compiled using Bun 1.3.12; see provenance.json and LICENSE.
export const TIERS = ["trivial", "simple", "moderate", "complex", "apex"];
export const HARNESSES = ["claude-code", "codex", "gemini", "kimi", "opencode", "pi", "hermes"];
export const VERSION = "gauge-0.3.0";
export function swarmTier(tier) {
  return tier === "apex" ? "complex" : tier;
}
export function classify(value) {
  if (typeof value !== "string" || !value.trim() || value.length > 1e5)
  ...
  if (/\bconvert (?:the )?(?:exact )?text quoted_value to (?:uppercase|lowercase)\b/.test(outsideQuotes) && !/\b(implement|design|build|write|add|diagnose|prove|compare|analy[sz]e|recommend)\b/.test(outsideQuotes))
    return result("trivial", ["bounded_lookup"]);
  const inquiry = has(/\b(derive|establish|characteri[sz]e|devise|formulate|research|invent|explore)\b/) || has(/\b(develop|design|create)\b[^.;!?]*\b(method|algorithm|framework|semantics|compressor|planner|protocol)\b/);
  ...
  if (inquiry && formal)
    return result("apex", ["research_proof_obligations"]);
  ...
  if (reasoning && transition && failure && coordination)
    return result("complex", ["cross_boundary_change"]);
  const resultOnly = has(/\b(return|report|copy|express|convert|extract|what is)\b/) && !reasoning && !has(/\b(function|helper|component|handler|script|endpoint|api|file|code|make|compare|evaluate|recommend|analy[sz]e|review|root cause|trade.offs)\b/);
  ...
  if (has(/^(return|extract|sort)\b/) && has(/\b(filename|basename|alphabetical|alphabetically)\b/))
    return result("trivial", ["bounded_lookup"]);
  const fieldLookup = lookup && has(/\b(value|field|property|position|count|literal)\b/) && !has(/\b(design|implement|build|create|develop|diagnose|debug|investigate|prove|derive|repair|fix|resolve|add|change|replace|update|compare|evaluate|recommend|analy[sz]e)\b/);
  if (fieldLookup)
    return result("trivial", ["bounded_lookup"]);
  ...
  const boundedChange = has(/\b(rename|change|replace|update|set)\b/) && has(/\b(label|name|text|count|constant|flag|heading|padding|expectation)\b/) && !has(/\b(design|migrate|synchroni[sz]e|aggregate|recovery|across|concurrently|concurrent|failover|diagnose|investigate|prove|implement|build)\b/);
  const feature = has(/\b(api|endpoint|persist\w*|submit\w*|load(?:ing)?|save[ds]?|store|navigation)\b/) && has(/\b(errors?|fail(?:s|ure)?|pending|empty|missing|deleted|duplicates?|unsaved|reject\w*|validation)\b/);
  if (boundedChange && !feature || bounded && !feature && !systemic && !breadth && !novelty)
    return result("simple", ["bounded_edit"]);
  ...
  if (proof && (reasoning && (research || systemic || feasibility) || research && feasibility))
    return result("apex", ["research_proof_obligations"]);
  if (novelty && reasoning && (systemic || domains >= 2))
    return result("apex", ["novel_reasoning", "cross_domain"]);
  if (systemic && reasoning)
    return result("complex", ["systemic_reasoning"]);
  ...
  if (reasoning && recovery && interacting && invariant)
    return result("complex", ["recovery_invariants"]);
  ...
  if (reasoning && has(/\b(rollback|fairness|fair|fair service|isolated)\b/) && has(/\b(versions|fleet|providers|sessions|tenants|services|rollout)\b/))
    return result("complex", ["cross_boundary_change"]);
  if (has(/\b(aggregate|synchroni[sz]ed|persist|reuse)\b/) && has(/\b(checks|preference|screen|form|api|reloads|error formats)\b/))
    return result("moderate", ["bounded_engineering"]);
  if (feature && reasoning)
    return result("moderate", ["bounded_engineering"]);
  if (!systemic && !breadth && has(/\b((?:pure|one|single|isolated|reusable) (?:reusable )?function|standard .* formula|single.file|existing .* field|(?:write|implement|add|create) (?:a |one |small |reusable |javascript |python )*function)\b/) && !has(/\b(parser|formats|ingestion|validation|recovery)\b/))
    return result("simple", ["bounded_edit"]);
  if (has(/\b(endpoint|api|client|form|parser|cache|ingestion|pagination|paging|ttl)\b/) && has(/\b(add|extend|create|normalize|wrap|build|implement)\b/))
    return result("moderate", ["bounded_engineering"]);
