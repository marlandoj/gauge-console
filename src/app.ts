...
type Assessment = ReturnType<typeof classify>;
...
const explanations: Record<Assessment['reasons'][number], string> = {
...
const harnessNames: Record<typeof HARNESSES[number], string> = {
  'claude-code': 'Claude Code', codex: 'Codex CLI', gemini: 'Gemini CLI',
  kimi: 'Kimi Code CLI', opencode: 'OpenCode CLI', pi: 'Pi', hermes: 'Hermes Agent'
};
const examples = {
...
const harness = element<HTMLSelectElement>('harness');
...
function text(id: string, value: string) { element(id).textContent = value; }
function title(value: string) { return value[0].toUpperCase() + value.slice(1); }

function updateMapping() {
  const name = harnessNames[harness.value as typeof HARNESSES[number]];
  const tier = lastAssessment?.tier;
  text('mapping', tier
    ? `${name} · ${title(tier)} → ${title(swarmTier(tier))}. ${tier === 'apex' ? 'Apex maps to complex in the resolver.' : 'Tier is unchanged in the resolver.'} Metadata only; no harness is run.`
    : `${name} · No tier to map. Harness choice does not change classification or run a harness.`);
}

function clearResult(state: string, heading: string, description: string) {
  lastAssessment = null;
...
function assess() {
  ...
  try {
    ...
    if (assessment.tier) needle.style.transform = `rotate(${-80 + TIERS.indexOf(assessment.tier) * 40}deg)`;
    document.querySelectorAll<SVGTextElement>('.dial-label').forEach(label => label.classList.toggle('active', label.dataset.tier === assessment.tier));
...
function queueAssessment() {
  ...
  error.hidden = true;
  task.removeAttribute('aria-invalid');
  if (!composing) timer = setTimeout(assess, 220);
}
...
element<HTMLFormElement>('task-form').addEventListener('submit', event => { event.preventDefault(); assess(); });
task.addEventListener('input', queueAssessment);
task.addEventListener('compositionstart', () => { composing = true; clearTimeout(timer); });
task.addEventListener('compositionend', () => { composing = false; queueAssessment(); });
task.addEventListener('keydown', event => {
  if ((event.ctrlKey || event.metaKey) && event.key === 'Enter' && !event.isComposing) { event.preventDefault(); assess(); }
});
...
exampleButtons.forEach(button => {
  ...
  button.addEventListener('click', () => {
    composing = false;
    const key = button.dataset.example as keyof typeof examples;
...
document.querySelectorAll<HTMLButtonElement | HTMLTextAreaElement | HTMLSelectElement | HTMLFieldSetElement>('[disabled]').forEach(control => { control.disabled = false; });
